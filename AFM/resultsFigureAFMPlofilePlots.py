...
"====CONFIG==="
excelNameI = "20220428_s2_QI_AFM_PSPIP2_hex120_GTA.xlsx"
excelNameP = "profilePlots.xlsx"
imageSheetA = "13.21.43.325"
#imageSheetB = "1027_957"
profileSheetA = "profile"
...
profileA = pd.read_excel(absolute_path_P, sheet_name=profileSheetA).to_numpy()#-mubgA
...
fig.set_size_inches(6,8) #width,length
gs2 = gridspec.GridSpec (3, 1,width_ratios=[1],height_ratios=[3,1,1])
...
cax1.set_clim(0, 20)
ax1.axis('off')
scalebar = ScaleBar(2000/IA.shape[1], "nm",length_fraction=0.3,width_fraction=1/30,color='w',frameon=False,location='lower right')#,label_formatter = lambda x, y:'')
...
line4, = ax3.plot(profileA[:,6],profileA[:,7],'y',linewidth=2,label='profile 4')
ax3.set_xlabel('x [nm]',fontsize=15)
ax3.set_ylabel('Height [nm]',fontsize=15)
ax3.legend(handles=[line1, line2, line3, line4],fontsize=12)
ax3.set_ylim([0, 25])
'''
...
line7, = ax5.plot(profileA[:,12],profileA[:,13],'k',linewidth=2,label='profile 7')
ax5.set_xlabel('x [nm]',fontsize=15)
ax5.set_ylabel('Height [nm]',fontsize=15)
ax5.legend(handles=[line5, line6, line7],fontsize=12)
ax5.set_ylim([0, 20])
'''
